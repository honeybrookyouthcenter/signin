...
namespace YouthCenterSignIn.Logic.Tests
{
    ...
    {
        [TestMethod]
        public void Person_HasValidPersonFieldsTest()
        {
            var person = new Person();

            Assert.IsFalse(person.HasValidPersonInfo(out string issues), "The person should not be valid when first created.");
            ...
            person.LastName = "Esh";
            Assert.IsFalse(person.HasValidPersonInfo(out issues), "The person should not be valid when first created.");
            ...
            Assert.IsFalse(person.HasValidPersonInfo(out issues), "The person should not be valid when first created.");
            Assert.AreEqual("Please enter your first name and address.", issues,
                "The first name and address are blank but the birth date is valid.");

            person.Address = "52 Evergreen St, Gordonville PA 1759";
            Assert.IsFalse(person.HasValidPersonInfo(out issues), "The person should not be valid when first created.");
            Assert.AreEqual("Please enter your first name.", issues,
                "The first name is blank.");

            person.FirstName = "James";
            Assert.IsTrue(person.HasValidPersonInfo(out issues), "The person should not be valid when first created.");
